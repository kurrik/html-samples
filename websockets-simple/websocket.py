...
class WebSocketHandler(tornado.web.RequestHandler):
    ...
    def _execute(self, transforms, *args, **kwargs):
        ...
        def format_header_key(key):
          ...
          number = int(REGEX_NONDIGIT.sub('', header))
          spaces = header.count(' ')
          return number / spaces
          
        def on_body_key(body_key_string):
          key1 = format_header_key("Sec-Websocket-Key1")
          key2 = format_header_key("Sec-Websocket-Key2")
          
          body_key_bytes = [c for c in body_key_string]
          challenge = struct.pack("!llcccccccc", key1, key2, *body_key_bytes)
          challenge_response = hashlib.md5(challenge).digest()
        ...
        self.request.connection.stream.read_bytes(8, on_body_key) 
